...
#   - S3 write: reports_frame format=parquet
...
class CreditBureauConnector:
    ...
    def __init__(self, spark_session: SparkSession, bureau: str = 'EXPERIAN'):
        """
    ...
    def batch_pull_credits(self, applicants_frame: DataFrame,
                           batch_size: int = 100) -> DataFrame:
        """
        Batch pull credit reports for multiple applicants.

        Args:
            applicants_frame: DataFrame with applicant data
            batch_size: Number of parallel requests

        Returns:
            DataFrame with credit reports
        """
        applicants_df = applicants_frame
    ...
    def cache_credit_reports(self, reports_frame: DataFrame,
                             cache_path: str):
        """
        Cache credit reports to S3 for reuse.

        Args:
            reports_frame: Credit reports DataFrame
            cache_path: S3 path for cache
        """
        reports_frame.write.format("parquet").mode("overwrite").save(cache_path)

    def get_cached_reports(self, cache_path: str,
                           max_age_days: int = 30) -> DataFrame:
        """
        ...
        Returns:
            DataFrame with cached reports
        """
        from datetime import timedelta

        cutoff = (datetime.utcnow() - timedelta(days=max_age_days)).strftime('%Y-%m-%d')

        cached_frame = spark.read.format("parquet").load(cache_path)

        # Filter by age
        cached_df = cached_frame
